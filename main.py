...
id_do_servidor = #ID do servidor do bot
id_cargo_atendente = #ID do cargo que vai atender nos tickets
token_bot = #Token API do bot / Não compartilhe
...
class Dropdown(discord.ui.Select):
    ...
    async def callback(self, interaction: discord.Interaction):
        if self.values[0] == "ajuda":
            await interaction.response.send_message("Se você precisar de ajuda, mande para mim!",ephemeral=True)
...
@tree.command(name="gojickets", description="Chame o Gojickets!!")
async def olamundo(interaction: discord.Interaction):
    await interaction.response.send_message(f"Olá {interaction.user.mention}!", ephemeral=True)

@tree.command(name="somaai", description="Fala uns número pro Gojickets somar!! (Não sei porque mas tá né, KKKK)")
@app_commands.describe(
    numero1 = "Primeiro número somado",
    numero2 = "Segundo número somado"
)
async def olamundo(interaction: discord.Interaction,numero1:int,numero2:int):
    numero_somado = numero1 + numero2
    await interaction.response.send_message(f"{numero_somado} | Tá ai fi!! O gojickets sabe matemática pô.", ephemeral=True)
